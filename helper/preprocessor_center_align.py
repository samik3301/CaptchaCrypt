...
def resizing_image(cropped_img):
    # img = cv2.imread(img_path)
    width = 200 #captcha original width - according to the keras implementation : width=200
    left_margin = (cropped_img.shape[1] - width) // 2
    right_margin = cropped_img.shape[1] - left_margin
    resized_image = cropped_img[:, left_margin:right_margin]
    return resized_image


if __name__ == "__main__":
    input_image_path = r'./data/indiagov_captchas/rrcfew.png'  # Change this to the path of your input image
    output_image_path = r'./data/transformed/margin.png'  # Change this to the desired output image path
    result_image = find_word_and_margins(input_image_path)
    
    resized_image = resizing_image(result_image)
